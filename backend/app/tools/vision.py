...
class VisionTool:
    ...
    async def describe_image(self, image_data: bytes) -> str:
        ...
        result = await self.analyze_image(
            image_data,
            prompt=(
                "Describe this image in detail. Include objects, "
                "people, setting, colors, mood, and any text visible."
            ),
    ...
    async def identify_objects(self, image_data: bytes) -> str:
        ...
        result = await self.analyze_image(
            image_data,
            prompt=(
                "List all objects, items, and entities visible in this "
                "image. Be specific and comprehensive."
            ),
    ...
    async def analyze_scene(self, image_data: bytes) -> str:
        ...
        result = await self.analyze_image(
            image_data,
            prompt=(
                "Analyze this scene. What is happening? Where is this? "
                "What is the context? What's the mood or atmosphere?"
            ),
            analysis_type="scene",
        )
